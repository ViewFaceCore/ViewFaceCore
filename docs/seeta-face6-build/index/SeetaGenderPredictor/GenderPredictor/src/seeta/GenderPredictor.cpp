...
namespace seeta{
    namespace v6{
        ...
        class GenderPredictor::Implement{
            public:
             ...
             bool PredictGender(const SeetaImageData &image, GENDER &gender) const;
            ...
            void set(GenderPredictor::Property property, double value) {
                switch (property) {
                    default:
                        break;
                    case GenderPredictor::PROPERTY_NUMBER_THREADS:
                    {
                        if (value < 1) value = 1;
                    ...
                    case GenderPredictor::PROPERTY_ARM_CPU_MODE:
                    {
            ...
            double get(GenderPredictor::Property property) const {
                switch (property) {
                    default:
                        return 0;
                    case GenderPredictor::PROPERTY_NUMBER_THREADS:
        ...
        GenderPredictor::Implement::Implement(const seeta::ModelSetting &setting) {
        ...
        GenderPredictor::Implement::Implement(const GenderPredictor::Implement &other) {
            *this = other;
            this->m_bench = this->m_bench.clone();
        }

       bool GenderPredictor::Implement::PredictGender(const SeetaImageData &image,GENDER &gender) const
       {
            ...
            // softmax
            float *_softmax_output = new float[_count];
            float _sum = 0;
            for (int i = 0; i < _count; ++i)
            {
                _softmax_output[i] = exp(output.data<float>(i));
                _sum += _softmax_output[i];
            }
            for (int i = 0; i < _count; ++i)
            {
                _softmax_output[i] /= _sum;
            }
            std::cout << "LOG: Predict result: ";
            for (int i = 0; i < _count; ++i)
            {
                if (i) std::cout << ", ";
                std::cout << _softmax_output[i];
                // std::cout << output[i];
            }
            std::cout << std::endl;
            delete[] _softmax_output;
            #endif // _DEBUG

            if (output.data<float>(0) > output.data<float>(1))
            {
                gender = FEMALE;
            }
            else
       ...
       GenderPredictor::GenderPredictor(const SeetaModelSetting &setting)
    ...
    int GenderPredictor::GetCropFaceWidth() const{
        return m_impl->m_param.alignment.width;
    }

    int GenderPredictor::GetCropFaceHeight() const{
        return m_impl->m_param.alignment.height;
    }

    int GenderPredictor::GetCropFaceChannels() const{
        return m_impl->m_param.alignment.channels;
    }

    bool GenderPredictor::CropFace(const SeetaImageData &image, const SeetaPointF *points, SeetaImageData &face) const {
        ...
        meanshape = seeta::resize(meanshape, { 109, 109 });
        auto cropped_face = seeta::crop_face(image, meanshape, seeta::Landmarks(points, 5), seeta::BY_LINEAR, { 128, 128 });
    ...
    bool GenderPredictor::PredictGender(const SeetaImageData &image, GENDER &gender) const
    {
       return m_impl->PredictGender(image, gender);
    }

    bool GenderPredictor::PredictGenderWithCrop(const SeetaImageData &image, const SeetaPointF *points, GENDER &gender) const
    {
        ...
        return PredictGender(cropped_face, gender);
    }

    void GenderPredictor::set(GenderPredictor::Property property, double value) {
        m_impl->set(property, value);
    }

    double GenderPredictor::get(GenderPredictor::Property property) const {
