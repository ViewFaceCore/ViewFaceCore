...
namespace seeta
{
	...
	public:
		...
		#ifdef SEETA_SUPPORT
		template <size_t _N>
		Landmarks(const SeetaPointF(&points)[_N]) : Landmarks(points, _N) {}
		Landmarks(const SeetaPointF *points, size_t _N)
			: points(_N)
		{
			for (int i = 0; i < _N; ++i) this->points[i] = points[i];
		}
		void copy_to(SeetaPointF *points, int N = -1)
		{
			if (N < 0) N = static_cast<int>(this->points.size());
			for (int i = 0; i < N; ++i) points[i] = this->points[i];
		}
		#endif
