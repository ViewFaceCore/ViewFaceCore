...
namespace seeta
{
	...
	public:
		Landmarks() {};
		Landmarks(const SeetaPointF *pts, size_t _N)
			: points(_N)
		{
			for (size_t i = 0; i < _N; ++i) {
				this->points[i].x = pts[i].x;
				this->points[i].y = pts[i].y;
			}
		}
		void copy_to(SeetaPointF *pts, int N = -1)
		{
			if (N < 0) N = static_cast<int>(this->points.size());
			for (int i = 0; i < N; ++i) {
				pts[i].x = this->points[i].x;
				pts[i].y = this->points[i].y;
			}
		}
		std::vector<PointF> points;
	};
